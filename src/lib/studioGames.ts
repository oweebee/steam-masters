...
export type StudioGameLink = { name: string; appid: string | null; hasCard: boolean };
...
export async function buildGameLinkMap(
  allNames: string[]
): Promise<Map<string, { appid: string; hasCard: boolean }>> {
  ...
  return new Map(games.map((g) => [g.name, { appid: g.id, hasCard: g.cards.length > 0 }]));
}

export function toStudioGameLinks(
  gameNames: string[],
  map: Map<string, { appid: string; hasCard: boolean }>
): StudioGameLink[] {
  return gameNames.map((name) => {
    const hit = map.get(name);
    return { name, appid: hit?.appid ?? null, hasCard: hit?.hasCard ?? false };
  });
}
