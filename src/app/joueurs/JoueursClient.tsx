          ...
          <section className="steam-library-vault">
            ...
            <div className="steam-library-grid">
              {library.map((card) => {
                ...
                return selectedPlayer.isSelf ? (
                  <article key={card.id} className={`steam-library-card border-2 ${RARITY_BORDER[card.rarity]}`}>{cardContent}</article>
                ) : (
                  <Link
                    key={card.id}
                    href={`/echanges?with=${selectedPlayer.id}&wantCard=${card.id}`}
                    onClick={() => setSelectedPlayer(null)}
                    className={`steam-library-card border-2 ${RARITY_BORDER[card.rarity]} hover:brightness-125 transition-[filter] cursor-pointer`}
                    title="Proposer un échange pour cette carte"
                  >{cardContent}</Link>
                );
              })}
