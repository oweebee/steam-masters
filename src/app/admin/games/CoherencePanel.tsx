  ...
  return <section className="max-w-7xl space-y-4">
    ...
    <div className="rounded-2xl border border-amber-900/80 bg-gradient-to-br from-stone-900 to-stone-950 p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-amber-100">Atelier de cohérence</h2>
          <p className="mt-1 text-sm text-stone-400">Contrôle des relations enregistrées dans la base. Aucun appel Steam pendant le scan local.</p>
        </div>
        <a href="/admin/logs" className="text-sm text-amber-300 underline">Journal de l&apos;application</a>
      </div>
      {report && <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-400"><span>{report.counts.games} jeux</span><span>{report.counts.studios} studios</span><span>{report.counts.dlcs} DLC</span><span>{report.ignoredCount} liens retirés</span><span className="text-stone-500">Scan du {new Date(report.scannedAt).toLocaleString("fr-FR")}</span></div>}
      <div className="mt-4 flex flex-wrap gap-2">
        <button className={button} disabled={locked} onClick={() => void local("scan")}>{busy && !importing ? "Analyse en cours…" : "Scanner la base locale"}</button>
        <button className={`${button} !border-amber-600`} disabled={locked} onClick={() => void local("repair-existing-links")}>⚙ Réparer les liens existants</button>
      </div>
      <p className="mt-2 text-xs text-stone-500">La réparation relie les fiches existantes sans appel externe. Les cas ambigus restent à examiner.</p>
      {busy && !importing && <div className="mt-3 flex items-center justify-between gap-2 rounded-lg border border-amber-800/50 bg-amber-950/30 px-3 py-2"><div className="flex items-center gap-2"><span className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-amber-400 opacity-75" /><span className="relative inline-flex h-2 w-2 rounded-full bg-amber-500" /></span><span className="text-xs text-amber-200">Analyse locale{report ? ` · ${report.counts.games} jeux / ${report.counts.studios} studios / ${report.counts.dlcs} DLC` : " de la base"} en cours…</span></div><span className="font-mono text-xs text-amber-500/70 tabular-nums">{scanElapsed}s</span></div>}
      {importing && importProgress && <div className="mt-3 space-y-2 rounded-lg border border-amber-900/60 bg-gradient-to-br from-stone-900 to-stone-950 px-3 py-3"><div className="flex items-center justify-between text-xs text-amber-200"><span className="font-medium">Import Steam en cours · {importProgress.index}/{importProgress.total}</span><span className="tabular-nums font-mono">{importProgress.total > 0 ? Math.round(importProgress.index / importProgress.total * 100) : 0} %</span></div><div className="h-1 overflow-hidden rounded-full bg-stone-800"><div className="h-full rounded-full bg-gradient-to-r from-amber-700 to-amber-400 transition-all duration-300" style={{ width: `${importProgress.total > 0 ? importProgress.index / importProgress.total * 100 : 0}%` }} /></div>{importProgress.current && <p className="truncate text-xs text-stone-400">▶︎ {importProgress.current}</p>}{lockWait > 0 && <p className="text-xs text-amber-400">⏳ Rate-limit Steam — reprise dans {lockWait}s</p>}<div className="flex items-center justify-between"><div className="flex gap-3 text-xs tabular-nums"><span className="text-emerald-400">✅ {importProgress.created} créé(s)</span><span className="text-stone-400">♻️ {importProgress.already} déjà OK</span><span className="text-red-400">❌ {importProgress.failed} échec(s)</span></div><button className={button} onClick={() => { stopped.current = true; setMessage("Arrêt demandé après l'élément en cours…"); }}>Arrêter</button></div></div>}
      {message && !importing && <p role="status" className="mt-3 text-sm text-amber-200">{message}</p>}
      {error && <p role="alert" className="mt-3 rounded border border-red-900 bg-red-950/40 px-3 py-2 text-sm text-red-200">{error}</p>}
    </div>

    {/* ─── ONGLETS RELATION ─── */}
    <div className="flex flex-wrap gap-1" role="tablist" aria-label="Filtrer par type de relation">
      <button role="tab" aria-selected={relation === "ALL"} className={`${button} ${relation === "ALL" ? "!border-amber-400 !text-amber-300" : ""}`} onClick={() => { setRelation("ALL"); setPage(0); }}>Tout voir ({report?.issues.length ?? "—"})</button>
      {(Object.entries(relations) as [Relation, string][]).map(([key, label]) => <button key={key} role="tab" aria-selected={relation === key} className={`${button} ${relation === key ? "!border-amber-400 !text-amber-300" : ""}`} onClick={() => { setRelation(key); setPage(0); }}>{label} · {report?.issues.filter((i) => i.relation === key).length ?? "—"}</button>)}
    </div>
    ...
    <div className="overflow-hidden rounded-xl border border-amber-900/60 bg-stone-950">
      ...
      <div className="flex flex-wrap gap-2 border-b border-stone-800 p-3">
        <input aria-label="Rechercher une anomalie" className="min-w-0 flex-1 rounded-lg border border-stone-700 bg-stone-900 px-3 py-2 text-sm text-white" placeholder="Nom, AppID, raison…" value={query} onChange={(e) => { setQuery(e.target.value); setPage(0); }} />
        <select aria-label="Filtrer les anomalies" className="rounded-lg border border-stone-700 bg-stone-900 px-3 py-2 text-sm text-white" value={status} onChange={(e) => { setStatus(e.target.value); setPage(0); }}>
          <option value="ALL">Tous les états</option>
          <option value="FAILED">Échecs / cas ambigus</option>
          <option value="LOCAL">Traitables localement</option>
          <option value="STEAM">Steam requis</option>
        </select>
      </div>

      {/* Zone A : actions sur le filtre courant */}
      <div className="border-b border-stone-800 bg-stone-900/50 px-3 py-3">
        <p className="mb-2 text-xs font-medium text-stone-400">Sur les <span className="text-amber-300">{filtered.length}</span> anomalies du filtre courant</p>
        <div className="flex flex-wrap gap-2">
          <button className={`${button} !border-blue-700 !text-blue-300`} disabled={locked || !filtered.filter((i) => i.method === "STEAM").length} onClick={() => void importSelection(filtered.filter((i) => i.method === "STEAM"))}>☁ Steam tous ({filtered.filter((i) => i.method === "STEAM").length})</button>
          <button className={`${button} !border-red-900 !text-red-300`} disabled={locked || !filtered.length} onClick={() => void ignoreAll()}>Retirer tout ({filtered.length})</button>
        </div>
        <p className="mt-2 text-xs text-stone-500">« Steam tous » importe/vérifie via Steam toutes les anomalies du filtre courant, une par une. « Retirer tout » masque définitivement ces liens (aucune carte joueur supprimée).</p>
      </div>

      {/* Zone B : actions sur la sélection */}
      <div className="border-b border-stone-800 px-3 py-3">
        <div className="mb-2 flex flex-wrap items-center gap-3">
          <p className="text-xs font-medium text-stone-400">Sur la sélection <span className="text-amber-300">{selected.length}/200</span></p>
          <button className="text-xs text-stone-500 underline hover:text-stone-300 disabled:opacity-40" disabled={locked} onClick={() => setSelected((current) => [...new Set([...current, ...visible.map((i) => i.key)])].slice(0, 200))}>+ sélectionner cette page</button>
          {selected.length > 0 && <button className="text-xs text-stone-500 underline hover:text-stone-300 disabled:opacity-40" disabled={locked} onClick={() => setSelected([])}>vider</button>}
        </div>
        <div className="flex flex-wrap gap-2">
          <button className={`${button} !border-blue-800`} disabled={locked || !steamSelected.length} onClick={() => void importSelection()}>☁ Steam sélection ({steamSelected.length})</button>
          <button className={button} disabled={locked || !studioSelected.length} onClick={() => void local("create-studios")}>Créer studios localement ({studioSelected.length})</button>
          <button className={`${button} !border-red-900 !text-red-300`} disabled={locked || !selected.length} onClick={() => void local("ignore")}>Retirer la sélection ({selected.length})</button>
        </div>
        <p className="mt-2 text-xs text-stone-500">Coche les lignes du tableau pour constituer ta sélection (max 200). Chaque bouton n&apos;utilise que les éléments sélectionnés compatibles avec l&apos;action.</p>
      </div>

      {/* Dépannage verrou Steam */}
      {!importing && <div className="border-b border-stone-800 px-3 py-2"><details><summary className="cursor-pointer text-xs text-stone-500 hover:text-stone-300">⚙ Dépannage — import bloqué ?</summary><div className="mt-2 flex items-start gap-3"><button className={`${button} !border-yellow-800 !text-yellow-400 shrink-0`} onClick={() => void fetch("/api/admin/coherence", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "clear-steam-lock" }) }).then(() => setError(""))}>🔓 Débloquer l&apos;import Steam</button><p className="text-xs leading-relaxed text-stone-500">Si un import précédent s&apos;est interrompu brutalement (erreur serveur, fermeture d&apos;onglet…), un verrou peut rester actif et empêcher tout nouvel import. Ce bouton le réinitialise sans risque.</p></div></details></div>}

      {/* Message courant pendant l'import */}
      {importing && message && <div role="status" className="border-b border-amber-900/50 bg-amber-950/30 px-3 py-2 text-sm text-amber-200">{message}</div>}
      ...
      <div className="flex items-center justify-between gap-2 border-t border-stone-800 p-3 text-xs text-stone-400"><button className={button} disabled={!currentPage} onClick={() => setPage(currentPage - 1)}>Précédent</button><span>{filtered.length} résultats · page {currentPage + 1}/{lastPage + 1}</span><button className={button} disabled={currentPage === lastPage} onClick={() => setPage(currentPage + 1)}>Suivant</button></div>
    </div>
