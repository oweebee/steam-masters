...
import type { BattleCard, BattleQuestion } from "@/lib/battle";

type OwnedCard = { id: string; rarity: string; atk: number; game?: { name: string; headerImage: string } | null; studio?: { name: string; avatarUrl: string | null } | null };
...
type Battle = {
  ...
  challengerDeck: BattleCard[]; opponentDeck: BattleCard[] | null;
...
function MiniCard({ card, hp, active }: { card?: BattleCard; hp?: number; active?: boolean }) {
...
function DeckPicker({ cards, selected, onChange }: { cards: OwnedCard[]; selected: string[]; onChange: (ids: string[]) => void }) {
  ...
  return <div className="battle-picker">
    ...
    <div className="battle-picker-list">
      {filtered.map((card) => {
        ...
        return <button type="button" aria-pressed={index >= 0} key={card.id} className={`battle-pick ${index >= 0 ? "battle-picked" : ""}`} onClick={() => onChange(index >= 0 ? selected.filter((id) => id !== card.id) : selected.length < 5 ? [...selected, card.id] : selected)}>
          {card.game?.headerImage || card.studio?.avatarUrl ? <img src={card.game?.headerImage ?? card.studio?.avatarUrl ?? ""} alt="" /> : <span>⚙️</span>}
          <span><strong>{name}</strong><small>{card.studio ? "Studio" : "Jeu"} · {card.rarity} · ATK {card.atk}</small></span>
          <b>{index >= 0 ? index + 1 : "+"}</b>
        </button>;
      })}
    </div>
...
export function BatailleClient() {
  ...
  useEffect(() => {
    ...
    void Promise.all([fetch("/api/joueurs"), fetch("/api/collection")]).then(async ([p, c]) => {
      setPlayers(await readJson(p)); setCards(await readJson(c));
    }).catch((err) => setError(err instanceof Error ? err.message : "Chargement impossible"));
  ...
  return <div className="battle-page">
    ...
    <p className="battle-rules">Les cartes restent dans ta collection. Victoire : 3 pièces + 25 XP · Défaite : 5 XP. Récompenses limitées aux 5 premiers combats du jour contre 5 adversaires différents (journée UTC).</p>
    {error && <p className="battle-alert" role="alert">{error}</p>}{notice && <p className="battle-notice" role="status">{notice}</p>}
    {showNew && <section className="battle-panel"><h2>Nouveau défi</h2><label className="battle-label">Adversaire<select value={opponentId} onChange={(event) => setOpponentId(event.target.value)}><option value="">Choisir un joueur</option>{players.filter((p) => p.cardCount >= 5).map((p) => <option value={p.id} key={p.id}>{p.username} · {p.cardCount} cartes</option>)}</select></label><h3>Ton deck, dans l’ordre</h3><DeckPicker cards={cards} selected={selected} onChange={setSelected} /><button disabled={busy || !opponentId || selected.length !== 5} className="battle-primary" onClick={() => void challenge()}>Envoyer le défi</button></section>}
    ...
    <section className="battle-section"><h2>Défis en attente <span>{pending.length}</span></h2>{pending.length === 0 && <p className="battle-muted">Aucun défi en attente.</p>}{pending.map((battle) => {
      ...
      return <article className="battle-panel battle-pending" key={battle.id}><div><h3>{incoming ? `${battle.challenger.username} te défie` : `Défi envoyé à ${battle.opponent.username}`}</h3><p className="battle-muted">{new Date(battle.createdAt).toLocaleString("fr-FR")}</p></div><div className="battle-actions">{incoming ? <><button className="battle-primary" onClick={() => { setAcceptId(acceptId === battle.id ? null : battle.id); setShowNew(false); setSelected([]); }}>Choisir mes 5 cartes</button><button className="battle-secondary" disabled={busy} onClick={() => void act(battle.id, "decline")}>Refuser</button></> : <button className="battle-secondary" disabled={busy} onClick={() => void act(battle.id, "cancel")}>Annuler</button>}</div>{acceptId === battle.id && <div className="battle-accept"><DeckPicker cards={cards} selected={selected} onChange={setSelected} /><button className="battle-primary" disabled={busy || selected.length !== 5} onClick={() => void act(battle.id, "accept", { cardIds: selected })}>Accepter le combat</button></div>}</article>;
    })}</section>
